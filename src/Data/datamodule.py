...
class SAMDataModule(pl.LightningDataModule):
    def __init__(self, 
                 ...
                 val_indices: list = [], # Indices of sample to use for validation. If empty list, use all indices in validatin set
        ...
        if len(val_indices) > 0:
            self.data_paths['val_images'] = [self.data_paths['val_images'][i] for i in range(len(self.data_paths['val_images'])) if i in self.val_indices]
            self.data_paths['val_masks'] = [self.data_paths['val_masks'][i] for i in range(len(self.data_paths['val_masks'])) if i in self.val_indices]
        print('\n Number of training images', len(self.data_paths['train_images']))
